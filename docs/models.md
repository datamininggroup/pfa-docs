...
The previous tutorials presented the breadth of PFA's features with simple calculations, but most real applications make use of advanced math, statistical models, and machine learning algorithms.  PFA provides these as a suite of built-in functions, organized in modules.  Complex models are not usually made by hand, but by a tool called a producer.  Once a model has been produced, it can be sent to another tool, called a consumer, whose purpose is to execute the scoring procedure implied by the model.  The producer and the consumer may be different or even unrelated software packages.

Producers are typically extensions to general-purpose statistical packages.  For instance, if an analyst has a favorite R package for producing predictive models, he or she can use that package as long as the model description file it generates can be translated into PFA.  The translator may be integrated into the R package or it may be a separate script.

Consumers are typically specialized PFA engines.  For instance, [Hadrian](http://hadrian.opendatagroup.com){:target="_blank"} is a scoring engine toolset from [Open Data Group](http://opendatagroup.com){:target="_blank"} that compiles PFA engines to JVM bytecode.  In the above example, the predictive model produced by an R package and converted into PFA would be uploaded to a server running Hadrian so that it can make predictions from large volumes of data.  

The general strategy is to use the most appropriate statistical package to produce a model, convert it to PFA, and then use a dedicated PFA consumer to integrate it into a production system.  This maximizes flexibility for the analysts and robustness for the production system.
